...
> -   **The Goal:** To process a challenging, **privacy-scrubbed** subset of the WIT dataset (upstream creators removed many **primary-subject** faces — a sparse INPUT haystack) and still find people wearing **eyeglasses** (not sunglasses) with faces at least 100x100 pixels. The qualitative galleries are those **finds**, not a claim that outputs are anonymized.
...
The goal of the pipeline was to take on a challenging "needle in a haystack" filtering task. The source, a subset of the `wikimedia/wit_base` dataset, is not only vast and uncurated, but was also **privacy-scrubbed** by its creators to remove images where human faces were the **primary subject**. That sparse INPUT haystack is the constraint. Our objective was to find the residual, less-prominent faces that remained: identifying and extracting all images containing human faces (at least 100x100 pixels) wearing eyeglasses, while explicitly excluding sunglasses. The final 125 targets and the qualitative galleries are the **needles the pipeline found** — public Wikimedia-derived QA crops, not a privacy regression. See [`CREDITS.md`](CREDITS.md) for Wikimedia / WIT attribution. This final dataset represents the successful result of that search and demonstrates the effectiveness of the underlying high-performance pipeline.
...
- **Data provenance:** Showcase crops and `image_url` pointers are Wikimedia Commons / WIT–derived; see [`CREDITS.md`](CREDITS.md).